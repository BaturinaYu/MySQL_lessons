...
INSERT INTO `another_answers` VALUES (2,6,15,'не здоровая','1'),(8,659,11,'35000','0'),(9,659,13,'нет','1'),(11,1017,15,'тяжёлая','0'),(12,1024,14,'близкие люди','0'),(15,1059,15,'стабильная','1'),(16,1061,14,'добрые люди','0'),(17,1174,14,'никто','0'),(19,1214,15,'затрудняюсь ответить','0'),(21,1275,10,'только не кредит','0'),(22,1275,11,'60000','0'),(23,1280,10,'не сталкивался с подобной проблемой','0'),(24,1280,14,'сослуживцы','0');
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `add_update_log` BEFORE UPDATE ON `questionnaires` FOR EACH ROW BEGIN
	IF NEW.title_id <> OLD.title_id THEN 
		INSERT INTO update_logs(table_name, field_name, old_value, new_value, updated_at)
		VALUES ('questionnaires', 'title_id', OLD.title_id, NEW.title_id, CURRENT_TIMESTAMP);
	END IF;
...
--
-- Temporary view structure for view `questions_list`
--
...
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `questions_list` AS SELECT 
 1 AS `number`,
 1 AS `question`,
 1 AS `answers`*/;
...
--
-- Temporary view structure for view `setting_another_answers`
--

DROP TABLE IF EXISTS `setting_another_answers`;
/*!50001 DROP VIEW IF EXISTS `setting_another_answers`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `setting_another_answers` AS SELECT 
 1 AS `title_id`,
 1 AS `question`,
 1 AS `answer`,
 1 AS `gender`,
...
--
-- Temporary view structure for view `setting_attitude`
--

DROP TABLE IF EXISTS `setting_attitude`;
/*!50001 DROP VIEW IF EXISTS `setting_attitude`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `setting_attitude` AS SELECT 
 1 AS `question_number`,
 1 AS `question`,
 1 AS `answer`,
 1 AS `count_respondents`,
 1 AS `attitude`*/;
SET character_set_client = @saved_cs_client;

--
-- Temporary view structure for view `setting_title`
--

DROP TABLE IF EXISTS `setting_title`;
/*!50001 DROP VIEW IF EXISTS `setting_title`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `setting_title` AS SELECT 
 1 AS `title_id`,
 1 AS `question`,
 1 AS `answer`,
 1 AS `gender`,
...
CREATE TABLE `update_logs` (
  ...
  `old_value` bigint unsigned DEFAULT NULL,
  `new_value` bigint unsigned DEFAULT NULL,
...
--
-- Final view structure for view `questions_list`
--
...
/*!50001 VIEW `questions_list` (`number`,`question`,`answers`) AS select `q`.`q_number` AS `q_number`,`q`.`text` AS `question`,`a`.`text` AS `answers` from ((`questions_answers` `qa` join `questions` `q` on((`qa`.`q_number` = `q`.`q_number`))) join `answers` `a` on((`qa`.`a_number` = `a`.`a_number`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `setting_another_answers`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `setting_another_answers` (`title_id`,`question`,`answer`,`gender`,`age`,`profession`) AS select `t`.`id` AS `id`,`q`.`text` AS `question`,`aa`.`text` AS `answer`,`t`.`gender` AS `gender`,`a`.`age` AS `age`,`p`.`name` AS `name` from ((((`another_answers` `aa` join `questions` `q` on((`aa`.`q_number` = `q`.`q_number`))) join `title` `t` on((`aa`.`title_id` = `t`.`id`))) join `age` `a` on((`t`.`age_id` = `a`.`id`))) join `professions` `p` on((`t`.`profession_id` = `p`.`id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `setting_attitude`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `setting_attitude` (`question_number`,`question`,`answer`,`count_respondents`,`attitude`) AS select `q2`.`q_number` AS `q_number`,`q2`.`text` AS `text`,`a`.`text` AS `text`,count(0) AS `count_respondents`,`a2`.`attitude` AS `attitude` from ((((`title` `t` join `attitude` `a2` on((`t`.`attitude_id` = `a2`.`id`))) join `questionnaires` `q` on((`t`.`id` = `q`.`title_id`))) join `questions` `q2` on((`q`.`q_number` = `q2`.`q_number`))) join `answers` `a` on((`q`.`a_number` = `a`.`a_number`))) group by `a2`.`id`,`q`.`q_number`,`q`.`a_number` order by `a2`.`id`,`q`.`q_number`,`q`.`a_number` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `setting_title`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `setting_title` (`title_id`,`question`,`answer`,`gender`,`age`) AS select `t`.`id` AS `id`,`q2`.`text` AS `question`,`a2`.`text` AS `answer`,`t`.`gender` AS `gender`,`a`.`age` AS `age` from ((((`title` `t` join `age` `a` on((`t`.`age_id` = `a`.`id`))) join `questionnaires` `q` on((`t`.`id` = `q`.`title_id`))) join `questions` `q2` on((`q`.`q_number` = `q2`.`q_number`))) join `answers` `a2` on((`q`.`a_number` = `a2`.`a_number`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2021-10-01 19:55:56
